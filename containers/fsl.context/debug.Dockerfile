...
FROM base-image as debug-staging
...
WORKDIR /dummy
COPY --from=debug-staging /fsl_build/fsl_conda_env.yml fsl_conda_env.yml
COPY --from=debug-staging /fsl_build/fslinstaller.py fslinstaller.py
COPY --from=debug-staging /usr/local/bin/directory-checksum /usr/local/bin/directory-checksum
