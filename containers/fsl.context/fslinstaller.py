...
def download_fsl_environment(ctx, local_env=None):
    """Habitually, here we download the environment file from the 
    web. For us, instead, we just get it from the command line
    """

    if local_env:
        fname = local_env
        ctx.environment_file = local_env
    else:
        build        = ctx.build
...
def parse_args(argv=None, include=None):
    ...
    helps = {
        ...
        'envfile'      : 'Use this environment instead of loading it from the web',
...
def main(argv=None):
    ...
    with tempdir(args.workdir, delete=False) as tmpdir:
        ...
        download_fsl_environment(ctx, args.envfile)
